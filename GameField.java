		...
		for (int i=0; i < width; i++){
    		for (int j=0; j < height; j++){
				...
				if (cur instanceof Mountain){
					boolean left=false;
					if (i > 0){
						left=(game.getGrid().getGridSlot(i-1,j) instanceof Road);
					}
					boolean right=false;
					if (i < width-1){
						right=(game.getGrid().getGridSlot(i+1,j) instanceof Road);
					}
					boolean up=false;
					if (j > 0){
						up=(game.getGrid().getGridSlot(i,j-1) instanceof Road);
					}
					boolean down=false;
					if (j < height-1){
						down=(game.getGrid().getGridSlot(i,j+1) instanceof Road);
					}
					boolean up_left=false;
					if ((i > 0)&&(j > 0)){
						up_left=(game.getGrid().getGridSlot(i-1,j-1) instanceof Road);
					}
					boolean up_right=false;
					if ((i < width-1)&&(j > 0)){
						up_right=(game.getGrid().getGridSlot(i+1,j-1) instanceof Road);
					}
					...
					if ((i < width-1)&&(j < height-1)){
						down_right=(game.getGrid().getGridSlot(i+1,j+1) instanceof Road);
					}
