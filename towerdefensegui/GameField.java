...
import game_tile.GameTile;
import game_tile.Mountain;
import game_tile.Road;
import game_tile.Tower;
...
public class GameField extends JPanel {
	...
	public GameField(GameManager game) {
		...
		try {
			roadImage = ImageIO
				.read(getClass().getResourceAsStream("/images/" + name + "093" + ".png"));
			mountainImage = ImageIO
				.read(getClass().getResourceAsStream("/images/" + name + "024" + ".png"));
		} catch (IOException exc) {
			exc.printStackTrace();
		}
		//////////////////////////
		...
		layout.setHorizontalGroup(
			layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
					...
					.addComponent(saveBut, javax.swing.GroupLayout.PREFERRED_SIZE, 100,
			...
			layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
					...
					.addComponent(backBut, javax.swing.GroupLayout.PREFERRED_SIZE, 33,
						javax.swing.GroupLayout.PREFERRED_SIZE)
					.addGap(20, 20, 20)
					.addComponent(saveBut, javax.swing.GroupLayout.PREFERRED_SIZE, 33,
						javax.swing.GroupLayout.PREFERRED_SIZE)
					.addGap(85, 85, 85))
		);
	...
	private void drawTowers(Graphics g) {
		for (int i = 0; i < game.getTowerManager().towerList.size(); i++) {
			game.getTowerManager().towerList.get(i).getImage()
				.paintIcon(this, g, game.getTowerManager().towerList.get(i).getLocX(),
					game.getTowerManager().towerList.get(i).getLocY());
			if (game.getTowerManager().towerList.get(i).hasTarget()) {
				double rad = Math.atan2(
					game.getTowerManager().towerList.get(i).getTarget().getLocY() + 32 - (
						game.getTowerManager().towerList.get(i).getLocY() + 32),
					game.getTowerManager().towerList.get(i).getTarget().getLocX() + 32 - (
						game.getTowerManager().towerList.get(i).getLocX() + 32));
				Graphics2D g2d = (Graphics2D) g.create();
				AffineTransform backup = g2d.getTransform();
				AffineTransform a = AffineTransform
					.getRotateInstance(rad, game.getTowerManager().towerList.get(i).getLocX() + 32,
						game.getTowerManager().towerList.get(i).getLocY() + 32);
				g2d.setTransform(a);
				game.getTowerManager().towerList.get(i).getTowerTurretImageFile()
					.paintIcon(this, g2d, game.getTowerManager().towerList.get(i).getLocX(),
						game.getTowerManager().towerList.get(i).getLocY());
			} else {
				game.getTowerManager().towerList.get(i).getTowerTurretImageFile()
					.paintIcon(this, g, game.getTowerManager().towerList.get(i).getLocX(),
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				...
				if (cur instanceof Road) {
					...
					g.drawImage(mountainImage, slotSize * i, slotSize * j, this);
				}

				if (game.getGrid().getGridSlot(i, j) instanceof Mountain
					&& ((Mountain) (game.getGrid().getGridSlot(i, j))).towerImage != null) {
					((Mountain) (game.getGrid().getGridSlot(i, j))).towerImage
						.paintIcon(this, g, game.getGrid().getGridSlotSize() * i,
							game.getGrid().getGridSlotSize() * j);
					((Mountain) (game.getGrid().getGridSlot(i, j))).turretImage
						.paintIcon(this, g, game.getGrid().getGridSlotSize() * i,
							game.getGrid().getGridSlotSize() * j);
		...
		for (int i = 0; i < game.getEnemyManager().enemyList.size(); i++) {
			...
			if (game.getEnemyManager().enemyList.get(i) instanceof AirEnemy) {
				if (game.getEnemyManager().enemyList.get(i).isHasSidePath()) {
					game.getEnemyManager().enemyList.get(i).enemySidePath
						.paintIcon(this, g, game.getEnemyManager().enemyList.get(i).getLocX() - 30,
							game.getEnemyManager().enemyList.get(i).getLocY());
				}
				game.getEnemyManager().enemyList.get(i).enemyImage
					.paintIcon(this, g, game.getEnemyManager().enemyList.get(i).getLocX(),
						game.getEnemyManager().enemyList.get(i).getLocY());
			} else {
				game.getEnemyManager().enemyList.get(i).enemyImage
					.paintIcon(this, g, game.getEnemyManager().enemyList.get(i).getLocX(),
			...
			//DRAW HEALTHBAR
			game.getEnemyManager().enemyList.get(i).enemyHealth
				.paintIcon(this, g, game.getEnemyManager().enemyList.get(i).getLocX() - 15,
					game.getEnemyManager().enemyList.get(i).getLocY() - 12);
	...
	private void drawProjectiles(Graphics g) {
		for (int i = 0; i < game.getTowerManager().towerCount; i++) {
			for (int j = 0; j < game.getTowerManager().towerList.get(i).getProjectileCount(); j++) {
				if (game.getTowerManager().towerList.get(i).getProjectilesSpawned().get(j).getLocY()
					< 580
					&& game.getTowerManager().towerList.get(i).getProjectilesSpawned().get(j)
					!= null) {
					if (game.getTowerManager().towerList.get(i).getProjectilesSpawned()
						.get(j).isAlive == false) {
						game.getTowerManager().towerList.get(i).getProjectilesSpawned().remove(j);
					} else {
						double rad = Math.atan2(
							game.getTowerManager().towerList.get(i).getProjectilesSpawned().get(j)
								.getTarget().getLocY() + 32 - (
								game.getTowerManager().towerList.get(i).getProjectilesSpawned()
									.get(j).getLocY() + 32),
							game.getTowerManager().towerList.get(i).getProjectilesSpawned().get(j)
								.getTarget().getLocX() + 32 - (
								game.getTowerManager().towerList.get(i).getProjectilesSpawned()
									.get(j).getLocX() + 32));
						if (game.getTowerManager().towerList.get(i).getProjectilesSpawned()
							.get(j).shootAir) {
							Graphics2D g2d = (Graphics2D) g.create();
							AffineTransform backup = g2d.getTransform();
							AffineTransform a = AffineTransform.getRotateInstance(rad,
								game.getTowerManager().towerList.get(i).getProjectilesSpawned()
									.get(j).getLocX() + 32,
								game.getTowerManager().towerList.get(i).getProjectilesSpawned()
									.get(j).getLocY() + 32);
							g2d.setTransform(a);
							game.getTowerManager().towerList.get(i).getProjectilesSpawned()
								.get(j).projectileImage.paintIcon(this, g2d,
								game.getTowerManager().towerList.get(i).getProjectilesSpawned()
									.get(j).getLocX(),
								game.getTowerManager().towerList.get(i).getProjectilesSpawned()
									.get(j).getLocY());
							...
							game.getTowerManager().towerList.get(i).getProjectilesSpawned()
								.get(j).projectileImage.paintIcon(this, g,
								game.getTowerManager().towerList.get(i).getProjectilesSpawned()
									.get(j).getLocX(),
								game.getTowerManager().towerList.get(i).getProjectilesSpawned()
									.get(j).getLocY());
	...
	private void drawExplosion(Graphics g) {
		for (int i = 0; i < game.getExplosion().size(); i++) {
			game.getExplosion().get(i).enemyImage
				.paintIcon(this, g, game.getExplosion().get(i).getLocX(),
					game.getExplosion().get(i).getLocY());
		}
	}
	...
	private void drawLayoutElements(Graphics g) {
		...
		g.drawString((game.getEnemyManager().getWaveNo() + 1) + "" + " / 4", 410,
			610); // will be updated
	...
	private void backButActionPerformed(java.awt.event.ActionEvent evt) {
		removeAll();
		revalidate();
		GameStage.getInstance().gameToMenu();
	}
